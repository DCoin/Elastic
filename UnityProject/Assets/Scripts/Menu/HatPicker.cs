...
public class HatPicker : MonoBehaviour {
	...
	private void OnGUI()
	{
		...
		GUI.Label (new Rect (Screen.width/2-(Screen.width/4.5f), 10, 100, 50), headingText, myStyle);

		//Draw menu options
		GUI.DrawTexture(new Rect(Screen.width-(Screen.width/6.0f),(Screen.width/70),(Screen.width/8),(Screen.width/8)), MenuOptions, ScaleMode.ScaleToFit, true, 0.0f);
	}
